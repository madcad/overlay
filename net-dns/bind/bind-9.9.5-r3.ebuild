...
# $Header: /var/cvsroot/gentoo-x86/net-dns/bind/bind-9.9.5-r3.ebuild,v 1.9 2014/07/19 20:34:30 klausman Exp $
...
IUSE="berkdb caps dlz doc dyndb filter-aaaa fixed-rrset geoip gost gssapi idn ipv6
...
src_prepare() {
	...
	use dyndb && epatch -p1 "${FILESDIR}"/${PN}-99-dyndb.patch
...
src_install() {
	...
	doins "${S}"/lib/isc/unix/errno2result.h
	insinto /usr/include/dns
	doins "${S}"/lib/dns/include/dns/rrl.h
	doins "${S}"/lib/dns/include/dns/forward.h
	doins "${S}"/lib/dns/include/dns/update.h
}
